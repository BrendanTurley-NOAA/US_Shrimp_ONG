...
mini_sz <- image_read('mini_SZ_plot.png')

setwd("~/R_projects/US_Shrimp_ONG/figures")

png('buffers_encounter.png', width = 16, height = 11, units = 'in', res = 300)
...
grid()
rug(x = seq(-.25,.05,.1), side = 2, ticksize = -.02, lwd = 1.5)
# mtext(2017)
text(hor, -.45, '\n horizon', srt = 90, adj = 1)
text(hor_twothirds, -.45, '\n partially visible', srt = 90, adj = 1, col = 'white')
text(hor_top, -.45, '\n not visible', srt = 90, adj = 1, col = 'white')
...
legend('bottomleft', legend=c(paste(13:16,'LA',sep='-'),
                              paste(17,'LA/TX',sep='-'),
                              paste(18:21,'TX',sep='-')),
       pch = pts, pt.bg = cols, bty = 'n', cex = 1.25)
...
grid.raster(mini_sz, x = .38, y = .17, width = .2)
